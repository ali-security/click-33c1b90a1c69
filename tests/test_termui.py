...
@pytest.mark.skipif(WIN, reason="Uses POSIX shell syntax and the echo command.")
def test_editor_filename_no_command_injection(monkeypatch, tmp_path):
    """A filename carrying shell metacharacters must not execute the
    injected command when a file is edited directly.
    """
    monkeypatch.chdir(tmp_path)
    # The marker is relative so that the injected command stays a single
    # path component and the file name remains creatable.
    target = tmp_path / 'note"; touch pwned.txt; echo ".txt'
    target.write_text("a\n")

    click.edit(filename=str(target), editor="echo")
